...
// called like this:
//      {% scheduleJSON dailySchedules, collections.allEvents %}
function renderJSON(dailySchedules, allEvents) {
    ...
    const data = {schedule :{
        ...
        conference: {
          ...
          , start: startTime.toISOString().split('T')[0] // self.event.date_from.strftime("%Y-%m-%d"),
          , end:  endTime.toISOString().split('T')[0] // self.event.date_to.strftime("%Y-%m-%d"),
    ...
    for(const [index, day] of dailySchedules.days.entries()) {
        console.log('day', day);
        const startTime = new Date(day.dateAndStartTime)
          , endTime =  new Date(day.dateAndEndTime)
          ;

        // for resource in talk.submission.resources.all()
        // if not resource.link
         const dayTalks = []
        for(const talkItem in day ){
            const talk = {};
            dayTalks.push(talk);
        }
        //const dayTalks = [
        ...
        const dayData = {
            ...
            'day_start':day.dateAndStartTime, // strftime("%Y-%m-%d"),
            'day_end': day.dateAndEndTime,
            'rooms': {
                'main': dayTalks
            }
        };
