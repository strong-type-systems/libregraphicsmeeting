...
async function getOnlineScheduleData() {
    const response = await fetch(SCHEDULE_URL)
      ...
      , scheduleData = JSON.parse(onlineData.scheduleString)
      ;
    console.log('scheduleData', scheduleData);
    onlineData.versionString = scheduleData.schedule.version
    const version = parseInt(onlineData.versionString, 10)

    onlineData.version = isFinite(version)
            ? version
...
function renderJSON(dailySchedules, allEvents, allHosts, onlineScheduleData) {
    ...
    if(scheduleString === onlineScheduleData.scheduleString)
